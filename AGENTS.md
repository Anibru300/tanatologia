...
13. 🟡 **Videollamadas migradas a JaaS (código listo, 2026-09-04)** — meet.jit.si limita las sesiones embebidas a **5 min** (modo demo); por eso se construyó la migración: Edge Function `jaas-token` (firma JWT RS256 por cita; solo participantes; profesional=moderator; sin secrets → 501 y fallback automático a meet.jit.si), `JaaSMeeting` en `JitsiMeetingRoom`, calidad 720p configurada. **Pendiente del cliente:** cuenta en jaas.8x8.vc (gratis 25 MAU) y entregar App ID + Key ID + clave privada → `supabase secrets set JAAS_APP_ID/JAAS_KID/JAAS_PRIVATE_KEY`. Detalle en `docs/avance-2026-09-04-videollamadas-jaas.md`. Test: `scripts/test-jaas.mjs` (10/10). Plan futuro si el volumen lo justifica: servidor Jitsi propio.
