...
- Los 3 portales usan el layout unificado `src/app/layouts/PortalLayout.tsx` (los layouts por rol definen `menuItems` y `quickNav` —accesos de la bottom nav móvil, máx. 4, filtrados del propio menuItems; badge "Pronto" para secciones ComingSoon; `matchPaths` para subrutas como `/sala`). Móvil: header con título de la sección actual + drawer lateral izquierdo (hamburguesa) + barra inferior de accesos rápidos con botón "Menú". `QuickExitButton` solo en el portal paciente (flota arriba de la bottom nav en móvil). Estados de carga: usar `Skeleton`/`SkeletonCards`/`SkeletonRows` (`src/components/ui/Skeleton.tsx`), no texto "Cargando...".
...
14. ✅ **(2026-09-06) Rotación de CRON_SECRET + soporte y contacto admin (migración 021, auditoría de correos):** el CRON_SECRET anterior quedó expuesto en el repo (migración 020, patrón cron.job); se rotó y movió al **vault** (`vault.create_secret`). `dispatch_user_email` vuelve a leer del vault; nuevo `dispatch_reminders_cron()` (SECURITY DEFINER) para que `cron.job` ya no lleve el secreto en texto plano. **Funciones nuevas**: `support-request` (paciente/profesional autenticado → inbox de soporte con reply_to + notificación in-app `support_request`; RLS de notifications no permite INSERT a authenticated — se inserta con service role) y `admin-contact` (solo admin → correo de marca a un usuario por `profile_id` + notificación in-app `admin_message`). UI: formulario de soporte en `PatientHelp`/`ProfessionalHelp`; botón "Contactar" (modal) en `AdminPatients` y `AdminProfessionals`. Auditoría completa: `docs/auditoria-2026-09-06-correos.md`.
15. ✅ **(2026-09-06) Sistema de Tutoriales (migración 022, reporte `docs/reporte-fase-19-tutoriales.md`):** tablas `tutorials`/`tutorial_views`, bucket Storage privado `tutorials` y páginas `/paciente/tutoriales`, `/profesional/tutoriales`, `/admin/tutoriales` (menú "Tutoriales" en los 3 portales). Visibilidad: usuarios solo ven `published` con audiencia permitida (signed URL 3600 s bajo demanda); admin gestiona todo (CRUD + subida con auto-miniatura + 6 estados). **Regla del cliente: ningún video se publica automáticamente** — flujo Subir→Revisar→Clasificar→Validar→Aprobar→Publicar. Los 5 videos en `VIDEOS TUTORIALES/` fueron revisados y auditados PII frame-a-frame: v2/v3/v4 editados con ffmpeg (blur pie de sidebar + cortes de PII/explorador de archivos/pausas; v4 contenía explorador personal que los revisores no vieron) y v1/v5 aprobados por **decisión expresa del dueño** pese a hallazgos (v1: PII real propia y de un tercero en todo el metraje; v5: flujo que falla en cámara — error ya corregido — y explorador personal); los 5 quedaron como `approved` (listos, NO publicados). Todos subidos a 1280w CRF 21 por el límite de ~50 MB del upload simple del API de Storage (el original de v1 eran 446 MB). **Ojo:** el API de Storage strippea el primer segmento de la key si coincide con el nombre del bucket — subir a `/object/tutorials/tutorials/{id}/video.mp4` guarda key `tutorials/{id}/video.mp4` (convención del frontend). Pendiente: visto bueno del dueño para publicar, re-grabación v1/v5, commit+push del frontend.
15. ✅ **(2026-09-04) Videollamadas en JaaS (8x8.vc)** — migración hecha y activa: meet.jit.si limitaba las sesiones embebidas a 5 min; la Edge Function `jaas-token` firma un JWT RS256 por cita (solo participantes, profesional=moderator; secrets `JAAS_APP_ID`/`JAAS_KID`/`JAAS_PRIVATE_KEY` en Supabase) y `JitsiMeetingRoom` usa `JaaSMeeting` del SDK con fallback automático a meet.jit.si si no hay token (sala manual del profesional). Calidad 720p configurada. Gratis hasta 25 MAU; después plan $99/300 MAU. Test: `scripts/test-jaas.mjs` 18/18 (verifica firma con clave pública) y `scripts/test-jaas-invoke.mjs` (replica el `functions.invoke` exacto del navegador). **Fix 2026-09-06: el fallback a meet.jit.si era silencioso** (sesiones reales morían a los 5 min sin aviso porque el navegador traía el bundle viejo en caché): `fetchJaasToken` ahora devuelve el motivo del fallo, `VideoCallExperience` reintenta una vez y muestra un `Alert` warning en el prejoin cuando usa la sala de respaldo. Pendiente: prueba real de llamada y verificar con el cliente que el `JAAS_KID` guardado no esté truncado (el formato completo termina en `-<NOMBRE_APP>`). Plan futuro si el volumen lo justifica: servidor Jitsi propio.
15b. ✅ **(2026-09-08) Sala de prueba de videollamada para admin:** nueva página `/admin/prueba-videollamada` (menú "Prueba de video") que permite al administrador entrar a una sala JaaS real —sin crear citas y sin límite de 5 min— para verificar cámara/mic/conexión. La Edge Function `jaas-token` ahora acepta `{ testRoom }` alternativo a `appointmentId`: valida formato de la sala (regex) y exige `profiles.role='admin'` (403 para otros roles; moderator=true). Frontend: `fetchJaasTestToken()` en `jaasService.ts` y prop `fetchToken` opcional en `VideoCallExperience` (debe ser estable, useCallback, para no refetchear). La página muestra el estado de configuración JaaS (badge + Alert con motivo del fallo), nombre de sala copiable y enlace "Abrir en pestaña nueva" con JWT (válido ~3 h) para probar entre dos dispositivos. **Fix CORS (mismo día):** `jaas-token` no respondía el preflight OPTIONS → las videollamadas reales caían en silencio al fallback de 5 min; ahora maneja OPTIONS + headers CORS (mismo patrón que las demás funciones). Función redeplegada; regresión `test-jaas.mjs` ampliada: 22/22.
15c. ✅ **(2026-09-08) Comparación de calidad con Daily.co (solo admin):** la misma página `/admin/prueba-videollamada` tiene una segunda tarjeta "Comparar con Daily.co" — crea una sala Daily Prebuilt (iframe fullscreen) vía la Edge Function `daily-test-room` (solo admin, secret `DAILY_API_KEY`; sin él responde 501 y la UI explica cómo configurarlo; la sala expira a las ~3 h). Propósito: decisión informada de migración de proveedor de video por calidad percibida (Jitsi/JaaS se veía pixelado por el umbral `minHeightForQualityLvl` mal anidado y VP8 por software; corregido en `JitsiMeetingRoom`). NO migra las salas reales — sigue siendo JaaS/Jitsi en citas. Pruebas: `test-daily.mjs` 6/6 (CORS, 401, 403, 405).
...
18. ✅ **(2026-09-17, v1.1.0-beta.1) UX móvil + rendimiento:** menú móvil rediseñado como **drawer lateral izquierdo** (antes: lista desplegable bajo el header) + **bottom nav** con 4 accesos directos por rol y botón "Menú" (nueva prop `quickNav` en PortalLayout); header móvil muestra la sección actual. Fuentes **Poppins woff2 auto-alojadas** (`public/fonts/`, subset latin, ~22 KB total vs ~250 KB TTF de gstatic) con preload de 400/600 en `index.html`. Imágenes con `loading="lazy" decoding="async"` donde aplica. 16 páginas de los 3 portales pasaron de "Cargando..." de texto a Skeletons. Verificado con capturas headless (Chrome + puppeteer-core en `.tools/review/`, gitignored).
17. ✅ **(2026-09-17) PWA instalable con actualizaciones automáticas (v1.1.0-beta.1):** la app (`/app/`) es ahora una PWA instalable en Android y iPhone/iPad. `vite-plugin-pwa` (generateSW + Workbox) precachea los assets versionados del build; `registerType: 'prompt'` con flujo estilo viajespro: el SW nuevo espera y `PwaUpdatePrompt` (`src/components/PwaUpdatePrompt.tsx`, montado en `main.tsx`) muestra banner "Nueva versión disponible → Actualizar" (skipWaiting + reload; la sesión de Supabase vive en localStorage y se conserva). Versión visible en el footer del sidebar de portales (`__APP_VERSION__` inyectada desde `package.json` en `vite.config.ts`); **bump de versión obligatorio en `package.json` en cada release** para que el banner anuncie la versión correcta. Manifest de la app: `start_url`/`scope`/`id` = `/app/`, `display: standalone`, iconos reutilizados de `/assets/images/icon-{192,512}.png` (raíz del sitio) y meta tags iOS (`apple-touch-icon` icon-180, `apple-mobile-web-app-capable`) en `platform/web/index.html`. El workflow de deploy no cambia: `dist` ya incluye `sw.js` + `manifest.webmanifest`. Reporte: `docs/reporte-pwa-2026-09-17.md`. El sitio estático de la raíz conserva su propio `manifest.json` sin SW (sin conflicto de scopes).
17b. ✅ **(2026-09-17, v1.3.0-beta.1) Banner de instalación + íconos de marca:** nuevo `InstallAppBanner` (fijo abajo, descartable por sesión) en las pantallas de acceso (login/registro, vía `RootLayout`) además del botón en portales — la app es instalable desde el primer contacto, no solo post-login. Hook compartido `useInstallPrompt.ts` (fast-refresh: hook solo en su archivo). Íconos PWA regenerados (`assets/images/icon-{192,512,180}.png`): ahora solo la marca (manos + brote) centrada al 58% sobre fondo crema `#F7F5F2` (antes era el logo completo con texto, ilegible en home screen); `favicon.ico` de raíz y de `platform/web/public/` regenerados con la marca (antes casi en blanco). **(2026-09-17, v1.2.0-beta.1)** Botón **"Instalar app"** (`src/components/InstallAppButton.tsx`) en el footer del sidebar/drawer de los 3 portales — solo con sesión iniciada; captura `beforeinstallprompt` (Android/Chrome) y muestra instrucciones paso a paso en iOS/Safari; se oculta si ya está instalada (display-mode standalone). Regla del cliente: **la app instalada solo contiene login + portales** — las páginas públicas (legales, sitio estático) se abren SIEMPRE en pestaña nueva (`StaticPageRedirect` usa `window.open`, enlaces públicos de `RootLayout` llevan `target="_blank"`). El menú móvil del sitio estático lleva "Iniciar sesión" hasta arriba (`.nav__auth` en `components.js`/`main.css`, solo visible en el breakpoint móvil).
